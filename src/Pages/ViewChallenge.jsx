...
const ViewChallenge = () => {
  ...
  if (!localChallenge) return <p className="text-center mt-10">Challenge not found.</p>;
      ...
      <Container>
        <div>
          ...
          <div className="flex flex-col md:flex-row bg-white rounded-2xl shadow-md overflow-hidden transform w-full mx-auto border border-gray-100 my-10">
            ...
            <div className="md:w-1/2 w-full p-6 flex flex-col justify-between bg-gray-50">
              <h2 className="text-2xl font-semibold text-gray-800 mb-4">{localChallenge.title}</h2>

              <div className="text-start space-y-2 text-gray-700 text-sm">
                <p><span className="font-medium text-gray-900">Category:</span> {localChallenge.category}</p>
                <p><span className="font-medium text-gray-900">Duration:</span> {localChallenge.duration} days</p>
                <p><span className="font-medium text-gray-900">Participants:</span> {localChallenge.participants}</p>
                <p><span className="font-medium text-gray-900">Created By:</span> {localChallenge.createdBy}</p>
                <p><span className="font-medium text-gray-900">Start Date:</span> {localChallenge.startDate}</p>
                <p><span className="font-medium text-gray-900">End Date:</span> {localChallenge.endDate}</p>
                <p><span className="font-medium text-gray-900">Target:</span> {localChallenge.target}</p>
                <p><span className="font-medium text-gray-900">Impact Metric:</span> {localChallenge.impactMetric}</p>
                <p><span className="font-medium text-gray-900">Description:</span> {localChallenge.description}</p>
              </div>
        ...
        <div className="grid justify-center grid-cols-1 lg:grid-cols-2 gap-6 py-10">
          {participants.map((participant) => (
            <ParticipantCard ey={participant._id} participant={participant} />
